# This is a small GUI interface to call xdem coregistration of a DEM to a reference, with unstable terrain.
...
def handle_error(exception_message, progress_bar_window, root):
    ...
    messagebox.showerror("Coregistration error", exception_message)
    root.quit()
...
def run_coregistration(file_paths, progress_bar_window, progress_bar, progress_bar_label, root):
    ...
    try:
        ...
        unstable_outlines = Vector(file_paths[2])
    ...
    progress_bar.after(0, update_progress_label, progress_bar_label, "Creating stable-terrain mask...")
    try:
        inlier_mask = ~unstable_outlines.create_mask(ref_dem) # This is a boolean numpy 2D array. Note the bitwise not (~) symbol
        progress_bar.after(0, update_progress_bar, progress_bar, 5)
    except Exception as error:
        exceptionlast = format_exc().splitlines()[-1]
    ...
    pipeline = coreg.NuthKaab() + coreg.Deramp(poly_order=1) + coreg.VerticalShift(vshift_reduc_func=np_nanmedian)




    progress_bar.after(0, update_progress_label, progress_bar_label, "Fitting and applying coregistration pipeline...")
    try:
        aligned_dem = pipeline.fit_and_apply(ref_dem, tba_dem, inlier_mask)
        progress_bar.after(0, update_progress_bar, progress_bar, 55)
    except Exception as error:
        exceptionlast = format_exc().splitlines()[-1]
        handle_error(f"There was an error fitting and applying the coregistration pipeline:\n\n{exceptionlast}\n\nNo output was generated, please correct the error and run the program again.\n\nClick OK to exit.", progress_bar_window, root)
    ...
    out_fn=sub("(\.[\w]{1,})$", r"_coreg\1", path_basename(file_paths[1])) # Add _coreg just before the file extension.
    out_path=path_join(path_abspath(out_dirpath), out_fn)

    try:
        aligned_dem.save(out_path)
        progress_bar.after(0, update_progress_bar, progress_bar, 10)
    except Exception as error:
        exceptionlast = format_exc().splitlines()[-1]
        handle_error(f"There was an error saving the output of coregistration:\n\n{exceptionlast}\n\nThe output could be missing or wrong, please correct the error and run the program again.\n\nClick OK to exit.", progress_bar_window, root)




    progress_bar_window.destroy()  # Close the progress window when the task is done
    messagebox.showinfo("Coregistration finished", "Coregistration finished successfully. The output file is located here:\n\n" + out_path + "\n\nClick OK to exit.")
    root.quit()
...
def start_process(file_paths, progress_bar_window, progress_bar, progress_bar_label, root):
    ...
    threading.Thread(target=run_coregistration, args=(file_paths, progress_bar_window, progress_bar, progress_bar_label, root), daemon=True).start()
...
def create_main_window():
    ...
    title_label = tk.Label(root, text="Coregistration of a DEM grid to a reference, using xdem", font=("Helvetica", 16))
    title_label.grid(row=0, column=0, columnspan=3, pady=5)

    info_label = tk.Label(root, text="Please select the DEM to use as reference, the DEM to be aligned, and the shapefile of unstable terrain, then click on \"Start coregistration\".", anchor="w")
    info_label.grid(row=1, column=0, columnspan=3, padx=10, pady=5, sticky="w")

    # File selectors
    file_selector_labels = ["Reference DEM", "DEM to be aligned", "Shapefile of unstable terrain"]
    entries = []
    ...
    process_button = tk.Button(root, text="Start coregistration", state=tk.DISABLED,
    ...
    progress_bar_window.title("Coregistration progress")
